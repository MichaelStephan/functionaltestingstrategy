 ...
 */
public class AcceptanceExpectations_v0 {
    private String postUrl;

    private String getUrl;

    public AcceptanceExpectations_v0(String postUrl, String getUrl) {
    ...
    public void givenValidInputToGetThenReturnsProduct() {
        // prepare
        Response postResponse = given().body("{\"name\":\"test\"}").and().contentType(ContentType.JSON).when().post(getPostUrl());
        postResponse.then().assertThat().statusCode(200);
        JsonPath postPath = new JsonPath(postResponse.asString());
        String id = postPath.get("id");
        String name = postPath.get("name");
        ...
        assertEquals(name, getPath.get("name"));
    }
}
