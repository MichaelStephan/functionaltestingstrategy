...
The goal of the automated contract tests is to protect any consumers from unforeseen non-compatible producer interface changes. The benefit of the given process is that the pact files are automatically generated and
no team has to do additional work except for maintaining its unit tests.
